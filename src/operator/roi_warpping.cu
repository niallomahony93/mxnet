 ...
 * \file roi_warpping.cu
 * \brief roi wrapping operator
 * \author Xingjian Shi
*/
#include "./roi_warpping-inl.h"
...
__global__ void ROIWarpForwardKernel(const int count, DType* top_data,
...
template<typename InterpOp, bool anti_aliasing, typename DType>
inline void ROIWarpForward(const Tensor<gpu, 4, DType> &out,
  ...
  CheckLaunchParam(dimGrid, dimBlock, "ROIWarpping Forward");
  cudaStream_t stream = Stream<gpu>::GetStream(out.stream_);
  ROIWarpForwardKernel<InterpOp, anti_aliasing, DType> << <dimGrid, dimBlock, 0, stream >> >(
...
template<typename InterpOp, bool anti_aliasing, typename DType>
__global__ void ROIWarpBackwardAccDataKernel(const int count, DType* bottom_data_diff,
...
__global__ void ROIWarpBackwardAccROIKernel(const int count, DType* bottom_roi_diff,
                                            const DType* bottom_data, const DType* bottom_rois, 
...
inline void ROIWarpBackwardAccData(const Tensor<gpu, 4, float> &in_data_grad,
  ...
  dim3 dimGrid(grid_dim_x, grid_dim_y);
  dim3 dimBlock(kMaxThreadsPerBlock);
  CheckLaunchParam(dimGrid, dimBlock, "ROIWarpping Backward Data");
  cudaStream_t stream = Stream<gpu>::GetStream(in_data_grad.stream_);
  ROIWarpBackwardAccDataKernel<InterpOp, anti_aliasing, float> <<<dimGrid, dimBlock, 0, stream >> >(
...
template<typename InterpOp, bool anti_aliasing>
inline void ROIWarpBackwardAccROI(const Tensor<gpu, 2, float> &in_roi_grad,
                                  const Tensor<gpu, 4, float> &out_grad,
  ...
  CheckLaunchParam(dimGrid, dimBlock, "ROIWarpping Data Backward");
  cudaStream_t stream = Stream<gpu>::GetStream(in_roi_grad.stream_);
  ROIWarpBackwardAccROIKernel<kMemUnitBits, InterpOp, anti_aliasing>
...
template<typename InterpOp, bool anti_aliasing, typename DType>
void ROIWarpForward(const Tensor<gpu, 4, DType> &out,
                           const Tensor<gpu, 4, DType> &data,
                           const Tensor<gpu, 2, DType> &bbox,
                           float spatial_scale,
                           bool explicit_batch,
                           int interp_kernel_size) {
  cuda::ROIWarpForward<InterpOp, anti_aliasing>(out, data, bbox, spatial_scale,
    explicit_batch, interp_kernel_size);
}

template<typename InterpOp, bool anti_aliasing>
void ROIWarpBackwardAccData(const Tensor<gpu, 4, float> &in_data_grad,
                                   const Tensor<gpu, 4, float> &out_grad,
                                   const Tensor<gpu, 2, float> &bbox,
                                   float spatial_scale,
                                   bool explicit_batch,
                                   int interp_kernel_size) {
  cuda::ROIWarpBackwardAccData<InterpOp, anti_aliasing>(in_data_grad, out_grad, bbox,
    spatial_scale, explicit_batch, interp_kernel_size);
}

template<typename InterpOp, bool anti_aliasing>
void ROIWarpBackwardAccROI(const Tensor<gpu, 2, float> &in_roi_grad,
                                  const Tensor<gpu, 4, float> &out_grad,
                                  const Tensor<gpu, 4, float> &data,
                                  const Tensor<gpu, 2, float> &bbox,
                                  float spatial_scale,
                                  bool explicit_batch,
                                  int interp_kernel_size) {
  cuda::ROIWarpBackwardAccROI<InterpOp, anti_aliasing>
...
template<>
Operator* CreateOp<gpu>(ROIWarppingParam param, int dtype) {
  Operator* op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new ROIWarppingOp<gpu, DType>(param);
  });
  return op;
}
