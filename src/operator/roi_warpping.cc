...
void ROIWarpForward(const Tensor<cpu, 4, DType> &out,
...
template<typename InterpOp, bool anti_aliasing, typename DType>
void ROIWarpBackwardAccData(const Tensor<cpu, 4, DType> &in_data_grad,
...
template<typename InterpOp, bool anti_aliasing, typename DType>
void ROIWarpBackwardAccROI(const Tensor<cpu, 2, DType> &in_roi_grad,
                           const Tensor<cpu, 4, DType> &out_grad,
                           const Tensor<cpu, 4, DType> &data,
                           const Tensor<cpu, 2, DType> &bbox,
                           float spatial_scale,
...
template<>
Operator *CreateOp<cpu>(ROIWarppingParam param, int dtype) {
  Operator* op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new ROIWarppingOp<cpu, DType>(param);
  });
  return op;
}

Operator *ROIWarppingProp::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
...
DMLC_REGISTER_PARAMETER(ROIWarppingParam);

MXNET_REGISTER_OP_PROPERTY(ROIWarpping, ROIWarppingProp)
...
"in the input data")
.add_arguments(ROIWarppingParam::__FIELDS__());
}  // namespace op
}  // namespace mxnet
