...
__global__ void fill_in_initial_indices(DType* out, int batch_num, int total_ele_num) {
  int tid = threadIdx.x + blockDim.x * blockIdx.x;
  if (tid < total_ele_num) {
    int row_id = tid % batch_num;
    out[tid] = DType(row_id);
  }
}
...
void ArgSortLastImpl(const mshadow::Tensor<gpu, 1, DType> &data,
  ...
  LOG(INFO) << "temp_storage_bytes=" << temp_storage_bytes << " num_items=" << num_items << " batch_num=" << batch_num;
  ...
  int grid_dim_x = (num_items + 512 - 1) / 512;
  dim3 dimGrid(grid_dim_x);
  dim3 dimBlock(512);
  CheckLaunchParam(dimGrid, dimBlock, "fill_in_initial_indices");
  fill_in_initial_indices << <dimGrid, dimBlock, 0, stream >> > (temp_values_in.dptr_, batch_num, num_items);
  if (is_ascend) {
