...
struct LocalCorrelationParam : public dmlc::Parameter<LocalCorrelationParam> {
  ...
  DMLC_DECLARE_PARAMETER(LocalCorrelationParam) {
    ...
    shape[0] = shape[1] = 0;
    DMLC_DECLARE_FIELD(no_padding).set_default(true)
    .describe("Whether we need to add padding to the lhs."
              " If no_padding is true, the rhs must have been padded and"
              " the \"valid\" correlation will be performed");
...
void LocalCorrelationForward_(const nnvm::NodeAttrs& attrs,
  ...
  bool no_padding = param_.no_padding;
...
inline bool LocalCorrelationShape(const nnvm::NodeAttrs& attrs,
  ...
  if (h1 == 0 && h2 != 0) {
    h1 = param_.no_padding ? (h2 - param_.dilate[0] * (param_.kernel[0] - 1)) : h2;
  } else if (h1 != 0 && h2 == 0) {
    h2 = param_.no_padding ? h1 + param_.dilate[0] * (param_.kernel[0] - 1) : h1;
  }
  if (w1 == 0 && w2 != 0) {
    w1 = param_.no_padding ? w2 - param_.dilate[0] * (param_.kernel[0] - 1) : w2;
  } else if (w1 != 0 && w2 == 0) {
    w2 = param_.no_padding ? w1 + param_.dilate[1] * (param_.kernel[1] - 1) : w1;
  }
